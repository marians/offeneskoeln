$(document).ready(function(){
  ...
  function truncateFulltext() {
    $('.attachment').each(function(n, attachment){
      ...
      $(attachment).find('.fulltext').each(function(n, fulltext){
        ...
        var truncated_html = OpenRIS.truncateText(fulltext_content, 350);
  ...
  function zoomThumbsClick(evt) {
    ...
    var full_width = 0;
    $('#' + evt.data.file_id + ' img').each(function(index){
      var img = $(this);
    ...
    $('#' + evt.data.file_id + ' .thumbsinner').css({width: full_width});
    // Animation zur Vergroesserung des Thumbnails-Containers
    $('#' + evt.data.file_id + ' .thumbs').animate({height: '825px'}, {duration: 300, easing: 'swing'});
    $('#' + evt.data.file_id + ' .thumbsinner').animate({height: '810px'}, {duration: 300, easing: 'swing'});
  }

  /**
   * Liest Informationen über alle Thumbnails zu allen Attachments
   * und befüllt thumbs_data - inaktiv, spart einen Request.
   */
  ...
  // inaktiv, spart einen Request
  /*OpenRIS.paperDetails(ok_document_url, function(data){
    //console.log(data);
